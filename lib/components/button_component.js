...
export class ButtonComponent extends extend_as("ButtonComponent").mix(Component).with() {
  ...
  constructor() {
    ...
    this.native_events            = [`![${this.click_event.join(',')}]`]; // TODO: expand this.click_event
    this.event_lock_for           = ["click"];                            // TODO: expand this.click_event
  ...
  afterInitialize() {
    ...
    if(this.get("lockable") == false)
      this.removeEventLock(this.click_event);
  }
