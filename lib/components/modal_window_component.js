...
export class ModalWindowComponent extends extend_as("ModalWindowComponent").mix(Component).with() {
  ...
  afterInitialize() {
    ...
    this._promise = new Promise((resolve,reject) => {
      ...
      if(self.get("close_on_background_click")) {
        self.event_handlers.add({ event: this.click_event, role: "self.background", handler: (self,event) => {
          if(event.target == self.findPart("background")) {
            self.hide();
            resolve(false);
          }
        }});
    ...
    //
    // TODO: Check if it's a problem in Javscript too.
    //document.onKeyDown.listen((e) => _processKeyDownEvent(e));
    this.dom_element.addEventListener("keydown", (e) => { if(e.keyCode == 27) self.hide() });
