export const ComponentDom = (ComponentDom) => class extends ComponentDom {
  ...
  initDomElementFromTemplate() {
    if(this.template != null) {
      ...
      this.attribute_names.forEach((a) => {
        ...
        if(this.get(a) != undefined) this._writePropertyToNode(a, this.get(a));
        // If not, we try to read that value from DOM and update Component object's attribute.
        else this.updatePropertiesFromNodes({ attrs: [a] });
      }, this);
    }
  }
  ...
  _writePropertyToNode(property_name) {
    ...
    property_els.forEach(function(property_el) {
      if(property_el != null) {
        var pa = property_el.getAttribute('data-component-attribute-properties');
        if(pa == null && property_el.getAttribute('data-component-property') == property_name) {
