export class ComponentDom {
  ...
  initDomElementFromTemplate() {
    if(this.template != null) {
      this.dom_element = new DOMParser().parseFromString(this.template.innerHTML, "text/html").documentElement;
  ...
  updatePropertiesFromNodes({ attrs=false, invoke_callbacks=false }={}) {
    ...
    attrs.forEach(function(a) {
      this._readPropertyFromNode(a);
      if(invoke_callbacks)
        this.invokeAttributeCallback(a);
    }, this);
  }

  /** Reads property value from a DOM node, updates Component's object property with the value */
  _readPropertyFromNode(property_name) {
    ...
    if(property_el != null) {
      ...
      if(pa == null) {
        ...
        s = s.replace(/^\s+/, "");
        s = s.replace(/\s+$/, "");
        this._assignPropertyFromNodeStringValue(property_name, s);
      }
      else {
        ...
        if(v != null)
          this._assignPropertyFromNodeStringValue(property_name, v);
      }
  ...
  _writePropertyToNode(property_name) {
    ...
    property_els.forEach(function(property_el) {
      if(property_el != null) {
        var pa = property_el.getAttribute('data-component-attribute-properties');
        if(pa == null)
          property_el.textContent = this.get(property_name).toString();
        else {
          var attr_property_name = this._getHtmlAttributeNameForProperty(pa, property_name);
          if(this.attributes[property_name] == null) {
            ...
            property_el.attributes[attr_property_name].value = this.attributes[property_name].toString();
            if(attr_property_name == "value")
  ...
  _assignPropertyFromNodeStringValue(property_name, s) {
    this.attributes[property_name] = this._convertStringValueToType(s);
  }

  /** Finds the template HtmlElement in the dom and assigns it to #template */
  _initTemplate() {
    return this.template = this.dom_element.ownerDocument.querySelector(`[data-component-template=${this.constructor.name}]`);
  }

  _assignRolesFromDomElement() {
    var roles_attr = this.dom_element.getAttribute('data-component-roles');
    if(roles_attr != null)
      this.roles = this.dom_element.getAttribute('data-component-roles').split(/,\s?/);
  }
