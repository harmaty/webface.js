  ...
  */
  _pickEvent(e) {
    ...
    if(this.event_handlers.map[name] != null) {
      ...
      // We don't always want to log these warning. If event is invoked on a parent (because events are published by children and go up the hierarchy),
      // chances are, there is no such event.
      if(data && data["log_warning"] != false)
        window.webface.logger.capture(`no event \`${name}\` found for role \`${publisher_roles}\`, in class \`${this.constructor.name}\``, { log_level: "WARN" });
    }
  }
